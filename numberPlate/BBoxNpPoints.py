...
CRAFT_DIR        = os.environ.get("CRAFT_DIR", os.path.join(NUMBER_PLATE_DIR, 'CRAFT-pytorch'))
...
def addoptRectToBbox(targetPoints, Bbox, distansesoffsetLeftMaxPercentage, offsetTopMaxPercentage,
    ...
    offsets = [distansesoffsetLeftPercentage, offsetTopPercentage, offsetRightPercentage,
    ...
    if not (overLeftTop) and not (overLeftBottom):
        if offsetLeftTop > offsetLeftBottom:
            ...
            points[1] = detectIntersectionNormDD(distanses[0]['matrix'], distanses[1]['matrix'], leftDistance,
                                                 0)
        else:
            points[1] = pLeftTop
            leftDistance = detectDistanceFromPointToLine(distanses[0]['matrix'], pLeftTop)
            points[0] = detectIntersectionNormDD(distanses[3]['matrix'], distanses[0]['matrix'], 0,
                                                 leftDistance)
    ...
    if not (overRightTop) and not (overRightBottom):
        if offsetRightTop > offsetRightBottom:
            ...
            points[2] = detectIntersectionNormDD(distanses[1]['matrix'], distanses[2]['matrix'], 0,
                                                 rightDistance)
        else:
            points[2] = pRightTop
            rightDistance = detectDistanceFromPointToLine(distanses[2]['matrix'], pRightTop)
            points[3] = detectIntersectionNormDD(distanses[2]['matrix'], distanses[3]['matrix'], rightDistance,
                                                 0)
...
class NpPointsCraft(object):
    ...
    def loadModel(self,
                  device="cuda",
                  is_refine=True,
                  trained_model=os.path.join(NUMBER_PLATE_DIR, 'numberPlate/Base/models/NpPointsCraft/mtl/craft_mlt_25k_2020-02-16.pth'),
                  refiner_model=os.path.join(NUMBER_PLATE_DIR, 'numberPlate/Base/models/NpPointsCraft/refiner/craft_refiner_CTW1500_2020-02-16.pth')
             ):
    ...
    def detect(self, image, targetBoxes, qualityProfile = [1,0,0]):
        all_points = []
